...
def generate_ensemble_plot():
    # 1. Dynamically compute the closest operational model run (00Z or 12Z)
    ...
    # 2. Re-route paths to match your 3-tier cascade folder strategy
    # structure: .../ensemble-track/wp/ECMWF/20260629/00Z/
    path = f"/Users/eknlau/VS_code/GHMWS/ensemble-track/wp/ECMWF/{date_folder_str}/{time_str}/"
    if not os.path.exists(path):
        os.makedirs(path)

    # Filenames matching the frontend request format
    ...
    print(f"[{datetime.now()}] Attempting download for {init_date} {time_str}...")
    ...
    try:
        ...
        print(f"Download failed/Data not ready yet: {e}. Will retry on next iteration.")
        return
    ...
    plt.title("ECMWF Ensemble Tracks - GHMWS", fontsize=16, fontweight='bold', pad=40, color='#1a237e')
    plt.text(0.5, 1.05, "360-hour Forecast", transform=ax.transAxes, ha='center', fontsize=13, fontweight='bold', color='#333333')
    plt.text(0.5, 1.02, f"Initial Time: {base_time_str}", transform=ax.transAxes, ha='center', fontsize=11, color='#546e7a')

    # CRITICAL CHANGE: Save the file dynamically and close the reference
    plt.savefig(output_png, bbox_inches='tight')
    plt.close(fig)
    print(f"[{datetime.now()}] Success: Saved plot asset to -> {output_png}")

# ==================== WHILE LOOP LOOP AREA =======================
if __name__ == "__main__":
    print("Starting automated ensemble plot generation task loop...")
    while True:
        generate_ensemble_plot()
        
        # Sleep for 12 hours (43200 seconds) before checking for updates again
